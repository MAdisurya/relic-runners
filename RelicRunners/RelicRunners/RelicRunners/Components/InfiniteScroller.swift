...
class InfiniteScroller {
    ...
    func generateBGSprites() -> Void {
        ...
        for i in 0...2 {
            ...
            bgFloor.size = CGSize(width: gameScene.size.width * 1.25, height: gameScene.size.height);
            bgWall.size = CGSize(width: gameScene.size.width * 1.25, height: gameScene.size.width * 0.5);
            fgRock.size = CGSize(width: gameScene.size.width * 1.25, height: gameScene.size.width / 5);
            
            // Set individual bg positions
            bgFloor.position = CGPoint(x: (gameScene.size.width * CGFloat(i)) * 1.25, y: 0);
            bgWall.position = CGPoint(x: (gameScene.size.width * CGFloat(i)) * 1.25, y: gameScene.size.width / 3);
            fgRock.position = CGPoint(x: (gameScene.size.width * CGFloat(i)) * 1.25, y: -gameScene.size.width / 3)
            bgFloor.zPosition = -1;
            fgRock.zPosition = 8;
    ...
    func checkToMoveBG() -> Void {
        ...
        for i in 0...2 {
            ...
            if (bgFloors[i].position.x < (gameScene.camera?.position.x)! - (gameScene.size.width * 1.75)) {
                sectionIndex += 1;
                moveBGWall(wall: bgFloors[i], to: CGPoint(x: (gameScene.size.width * 1.25) * (sectionIndex+2), y: 0));
